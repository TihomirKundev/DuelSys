...
namespace Staff_app
{
    public partial class CRUDTournament : Form
    {
        ...
        {
            if(String.IsNullOrEmpty(IDTxt.Text) || String.IsNullOrEmpty(SportTypeTxt.Text) || String.IsNullOrEmpty(TourSysTxt.Text) || String.IsNullOrEmpty(MinPlayersTxt.Text) || String.IsNullOrEmpty(MaxPlayersTxt.Text) || String.IsNullOrEmpty(LocationTxt.Text) || String.IsNullOrEmpty(GamesTxt.Text))
            {
            ...
            }else if(SportTypeTxt.Text != "Badminton")
            {
                MessageBox.Show("For now you can only choose badminton!");
                SportTypeTxt.Text = "Badminton";
            ...
            else
            {
                ...
                if(exc == 0)
                {
                    ...
                    {
                        Upcoming_tournament upcoming_Tournament = new Upcoming_tournament(id, DescTxt.Text, StartDatePicker.Value, EndDatePicker.Value, min, max, LocationTxt.Text, games, new List<People>());
                        string result = tournamentManager.CreateUpcoming(upcoming_Tournament);
                        if (result == "Complete")
                        {
                            this.Close();
        ...
        private void UpdateBtn_Click(object sender, EventArgs e)
        {
            if(StartDatePicker.Visible == false)//Ongoing
            {
                ...
                else
                {
                    ...
                    tournamentManager.Update(new Ongoing_tournament(id,DescTxt.Text,StartDatePicker.Value,EndDatePicker.Value,min,max,LocationTxt.Text,games,new Dictionary<People, int>(), new List<Game>()));
                    this.Close();
                    MainForm mainForm = new MainForm();
                    mainForm.Show();
                }
                ...
                else
                {
                    ...
                    if(err == 0)
                    {
                        ...
                        if(games == 1 || games == 3 || games == 5)
                        {
                            ...
                            int min = Convert.ToInt32(MinPlayersTxt.Text);
                            int max = Convert.ToInt32(MaxPlayersTxt.Text);
                            tournamentManager.Update(new Upcoming_tournament(id, DescTxt.Text, StartDatePicker.Value, EndDatePicker.Value, min, max, LocationTxt.Text, games, new List<People>()));
                            this.Close();
                            MainForm mainForm = new MainForm();
                            mainForm.Show();
                        }
